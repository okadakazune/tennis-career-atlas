...
import { ROOT, resolveFromRoot } from "./lib/paths.mjs";
...
async function main() {
  ...
  try {
    ...
    console.warn(`[data:update-latest] Failed to resolve player ids: ${error.message}`);
    process.exit(0);
  }

  const balldontlieIds = Object.values(mapping).map(Number).filter(Number.isFinite);
  if (balldontlieIds.length === 0) {
    console.warn("[data:update-latest] No BallDontLie player mappings available. Skipping rankings fetch.");
    process.exit(0);
  }
  ...
  for (const date of rankingDates) {
    try {
      ...
      const merged = mergeDeltaRecords(await loadExistingWeek(filePath), incoming);
  ...
  console.log(
    `[data:update-latest] Done. weeksUpdated=${weeksUpdated}, latestWeek=${latestWeek ?? "n/a"}`,
  );
}

main().catch((error) => {
  console.warn(`[data:update-latest] Unexpected error: ${error.message}`);
  process.exit(0);
});
